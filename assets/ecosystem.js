(function () {
  ...
  function getValidationPack(content) {
    return content.validationPack || {
  ...
  function renderValidationPack(content) {
    const pack = getValidationPack(content);
    const tableRows = pack.tests.map((test) => el("tr", {}, [
      el("td", { text: test.area }),
      el("td", { text: test.scenario }),
      el("td", { text: test.evidenceDate }),
      el("td", { text: test.outcome }),
      el("td", { text: test.evidence })
    ]));

    return el("section", { className: "ecosystem-section ecosystem-validation-pack reveal" }, [
      el("h2", { className: "section-title", text: pack.title }),
      el("p", { className: "ecosystem-pack-intro", text: pack.intro }),
      el("div", { className: "ecosystem-validation-summary" }, pack.summary.map((item) => el("article", { className: "ecosystem-validation-summary-card" }, [
        el("span", { className: "ecosystem-summary-label", text: item.label }),
        el("strong", { text: item.value }),
        el("p", { text: item.note })
      ]))),
      el("h3", { text: "Test Evidence Matrix" }),
      el("div", { className: "ecosystem-table-wrap" }, [
        el("table", { className: "ecosystem-test-table" }, [
          el("thead", {}, [
            el("tr", {}, [
              el("th", { text: "Area" }),
              el("th", { text: "Scenario" }),
              el("th", { text: "Data evidenza" }),
              el("th", { text: "Esito" }),
              el("th", { text: "Evidenza sintetica" })
            ])
          ]),
          el("tbody", {}, tableRows)
        ])
      ]),
      el("div", { className: "ecosystem-readiness-grid" }, [
        el("article", {}, [
          el("h3", { text: "Operational Readiness" }),
          renderList(pack.readiness)
        ]),
        el("article", {}, [
          el("h3", { text: "Known Limits / Guardrail" }),
          renderList(pack.guardrails)
        ])
      ]),
      el("p", { className: "ecosystem-source-note", text: "Fonti sanitizzate: " + pack.sources.join(", ") + "." })
    ]);
  }
